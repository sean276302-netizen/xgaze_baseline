...
def main(image_path, txt_path, face_3D_generic_model, face_model_ratio, camera_matrix, camera_distortion):
...
if __name__ == '__main__':
    ...
    camera_matrix, camera_distortion = calibration.load_calibration_from_file(f"test_hw/calibration/mono.xml")
    
    txt_path = os.path.join("test_hw/pred_PoG/batch", "p76_1.txt")
    if not os.path.exists(os.path.dirname(txt_path)):
        os.makedirs(os.path.dirname(txt_path))
    if os.path.exists(txt_path):
        os.remove(txt_path)

    image_paths = [f"E:/hw_test_dataset_B/p76/p76_154439_{i:04d}.jpg" for i in range(39)]
    ...
    progress_bar = tqdm(image_paths, desc="Processing images", unit="image")
    for image_path in progress_bar:
        main(image_path, txt_path, face_model_std, face_model_ratio, camera_matrix, camera_distortion)