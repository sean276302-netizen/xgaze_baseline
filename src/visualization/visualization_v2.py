...
def visualize_history(history_PoG, screen_width, screen_height):
    range_scale = 0.9
    history_percentile = np.percentile(history_PoG,
                                       [100 * (1 - range_scale) / 2, 100 - 100 * (1 - range_scale) / 2],
                                       axis=0)  # get percentile
    history_average = np.mean(history_percentile, axis=0)
    ...
    # input(f'{np.array(history_PoG)[:, 0]}')
    # 绘制预测点（蓝色点）
    plt.scatter(np.array(history_PoG)[:, 0], np.array(history_PoG)[:, 1], color='blue', s=20, label='History Points')
    plt.plot(history_average[0], history_average[1], 'go', markersize=10, label='Mean History Point')
    plt.plot(screen_width/2, screen_height/2, 'ro', markersize=10, label='Screen Center')
    ...
    # 显示图像
    cv2.imshow('History Gaze Points', img)
    cv2.waitKey(1)  # 等待1毫秒，确保图像显示
